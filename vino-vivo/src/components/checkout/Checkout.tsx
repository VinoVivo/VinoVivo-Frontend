        ...
        <div className="container mx-auto p-4 mt-32 flex">
            <div className="grid grid-cols-1 gap-4 flex-grow">
                {updatedCartItems.map((item) => (
                    <div key={item.id} className="border border-gray-200 p-4 rounded-md mb-4 flex items-center justify-between">
                        <div className='flex'>
                        ...
                        <div className='space-y-2'>
                            <div className='flex items-center'>
                                <button className="h-5 w-5 border border-textTypograph rounded flex items-center justify-center" onClick={() => handleQuantityChange(item.id, item.quantity - 1)}>-</button>
                                <p className='text-lg mr-2 ml-2 text-violeta font-medium'>{item.quantity}</p>
                                <button className="h-5 w-5 border border-textTypograph rounded flex items-center justify-center" onClick={() => handleQuantityChange(item.id, item.quantity + 1)}>+</button>
                            </div>
                            <button className="text-primary font-bold" onClick={() => handleRemoveItem(item.id)}>Eliminar</button>
                        </div>
                    </div>
                ))}
            </div>
            <div className="w-1/4 ml-8 border border-gray-200 rounded-md p-4 flex flex-col">
                ...
                {updatedCartItems.map((item) => (
                    <div className='flex justify-between'>
                        <p className='text-sm p-2' key={item.id}>{item.name}</p><p className='text-sm p-2 text-violeta font-medium'>x{item.quantity}</p>
                    </div>
                ))}
