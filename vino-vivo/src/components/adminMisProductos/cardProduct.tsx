...
const CardProduct: FC<ICardProduct> = ({ product, textButton, icon, href, onclick }) => {
  return (
    <div key={product.id} className="bg-card rounded-lg border border-gray-200 p-6 w-full sm:w-64">
      <div className="min-h-[40px] min-w-[40px]  flex justify-end">
        {icon && <button onClick={onclick} className='text-2xl text-destructive hover:text-primary '>{icon}</button>}
      </div>
      <Link href={`/detail/${product.id}`}>
        <Image src={product.image} alt={product.name} width={200} height={200} className="w-full h-auto transform transition-transform duration-300 hover:scale-105" />
      </Link>
